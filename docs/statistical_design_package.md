...
Step 2.5 的生成物格式已由 `contracts/step25_artifact_contract.md` 冻结。该 contract 明确了 `analysis_spec.R`、`config/config.json`、`data/stan_input_*.json`、`outputs/*.json` 以及 Step 3 `engine_package` snapshot 之间的结构边界。

统计设计人员可以修改 `analysis_spec.R` 中的参数级设置，例如 borrowing 参数、trimming、survival cut points、MCMC 设置和 diagnostics thresholds。但不得静默改变 Stan input JSON schema。尤其是 Bayesian engine 直接消费的 `data/stan_input_binary.json`、`data/stan_input_continuous.json` 和 `data/stan_input_survival.json` 必须继续匹配 `models/*.stan` 的 `data` block。

若模型结构改变导致 Stan `data` block、Stan input JSON 字段或 `summary_output.json` schema 变化，必须更新 `contracts/step25_artifact_contract.md` 的 contract version，并重新同步 Step 3 `engine_package/data/*.json` 和 `engine_package/expected_outputs/*.json`。
